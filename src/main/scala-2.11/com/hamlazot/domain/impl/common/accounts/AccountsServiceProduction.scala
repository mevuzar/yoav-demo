...
package domain.impl.common.accounts

import java.util.UUID

import com.hamlazot.DataDSL.DataStoreRequest
import com.hamlazot.domain.contract.common.accounts.{AccountRepositoryF, AccountsService}
import AccountRepositoryF.AccountDataOperations.{delete, query, store, updateMail}
 ...
 */
trait AccountsServiceProduction extends AccountsService with AccountsAggregate with LazyLogging{
  ...
  protected def updateAccountMail(id: UUID, mail: String): Future[UserAccount] = {
    ...
    val dbLog = runFC(script)(dbLogger).left.get
    dbLog.map(log => {

      logger.info(log)
    })

    val dbResult = runFC(script)(dbDriver).right.get
  ...
  protected def deleteAccount(accountId: java.util.UUID): Future[Unit] = {
    ...
    val dbResult = runFC(theQuery)(dbDriver)
    dbResult.right.get
  }
